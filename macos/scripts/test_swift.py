...
source = (ROOT / 'Tests/BrisaCoreTests/AccountTests.swift').read_text()
...
for name in [*names, 'full-suite']:
    ...
    if name != 'full-suite':
        command += ['--filter', 'AccountTests/' + name]
    ...
    with log.open('wb') as stream:
        ...
        try:
            ...
            for pid, executable in owned:
                if 'BrisaPackageTests' in executable:
                    subprocess.run(['sample', str(pid), '1', '1', '-file', str(OUT / (name + '.sample.txt'))],
