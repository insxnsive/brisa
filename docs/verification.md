...
The complete offline runner passed: 41 packaging tests; 77 backend passes and one elevation-only skip; the Go suites; 23 core tests; 85 navigation, 150 appearance and 57 lifecycle assertions; and C#–Node integration. Independent source review found no concrete blocker in the production fix and initial gate. The first CI run exposed a probe lifetime assumption: elevated inspection can retain an idle worker after answering requests, so the probe must validate replies and stop only its own Node child, as the real C# host does, instead of waiting for stdin EOF to end the service. A persistent-peer regression failed before this harness correction and passes afterward; the corrected probe still rejects the original beta.7 payload. The local packaged UI passed 232 assertions across both themes, account fixture states and both window sizes at actual 125% scaling. These checks establish production initialization, not a real Proton login or live routing. The existing installation and tunnels were left untouched.
...
- [Branch CI](https://github.com/insxnsive/brisa/actions/runs/36137102438) and [tag/release CI](https://github.com/insxnsive/brisa/actions/runs/36140081942) passed. The Windows runners passed all 78 backend tests with no skips, including the real elevated Administrators-owner fixture, plus the packaging, Go and C# suites.
- All eight public assets were downloaded without authentication and matched GitHub's recorded sizes and SHA-256 digests. The feed, package manifest and bundled/separate source verified. All 178 expected tracked source files matched the release commit.
- Every runtime file in the tested CI portable build matched the published portable build, including all 479 executable/DLL files. Only the source ZIP and its containing checksum manifest differed between builds; the published source was verified separately.
- The real Velopack `GithubSource`, without an access token, selected beta.7 for a beta.6 test locator and downloaded it with a matching hash. Current-version reinstall and downgrade checks passed. Both CI runs also applied a disposable portable `0.0.1 → 0.1.0-beta.7` update, rendered before and after, and preserved a test file outside the versioned app directory. No installed application was updated.
...
The CI-built executable passed 16 no-network UI combinations and 464 assertions: both themes, signed-in/out fixtures, 460×540 / 440×520 DIP sizes, and actual 100%/125% Windows scaling. The checks covered Account input and Back, Settings expansion/scrolling/Cancel/Save, Advanced diagnostics and Route selection. Page motion settled without looping, and inspected captures showed readable controls without unintended overlap or clipped footer actions.

It also passed 41 failed-startup assertions: both themes, primary Exit and window-close paths at actual 100%/125% scaling, Settings Save returning to the same error screen without losing its message, normal process termination, and unchanged pre-existing Brisa process identities. All four exit cases finished in under a second. Final 125% and failed-startup checks ran on a disposable Win32 desktop without switching the user's input desktop, after the initial harness exposed foreground-input interference. Runtime-file equality transfers this evidence to the published package.

No real credentials/session/profile contents were read. Independent C# and storage follow-up reviews found no remaining concrete blocking security or logic issue. All new and matching destinations are opened writable and flushed before held originals are removed; fault fixtures cover partial deletion and retry, not a physical power-loss test. The installed app and existing tunnel were not replaced or stopped. Clean-machine installation, real elevated startup/migration, higher-DPI acceptance, packaged two-factor acceptance, WireSock terms and trusted signing remain separate open gates. Portable and no-network fixture acceptance do not replace those remaining stable-release gates.
