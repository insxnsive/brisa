...
`tools/proton-confgen/internal/macengine` owns a WireGuard device and an isolated userspace IPv4/IPv6 stack. It supports unicast TCP and UDP through explicit inner addresses and peer routes, with A and AAAA lookup through configured in-tunnel DNS servers. Dials require an explicit `tcp4`, `tcp6`, `udp4` or `udp6` family, and DNS answers must match that family and a declared route. Scoped, link-local and IPv4-mapped addresses are rejected. It never falls back to the host network or resolver for forwarded connections. Configuration is typed and validated; errors do not echo private keys or raw configuration. Close cancels and joins pending operations, closes owned connections, and prevents late dial results from escaping.

This remains a **loopback-only development transport**. Connect stays disabled. It is not a host-wide VPN, an application filter, a system DNS policy, or an IPv6 leak-prevention mechanism. No host interface, route or resolver is installed.

Em português: esta etapa testa IPv4 e IPv6 apenas entre pares sintéticos dentro do processo. O botão Conectar continua desativado; o aplicativo não protege o tráfego do Mac nem altera as rotas ou o DNS do sistema.

The separate `brisa-tunnel-check --self-test` executable generates fresh synthetic keys and exchanges IPv4 and IPv6 TCP payloads, distinct UDP datagrams and A/AAAA `.invalid` DNS answers between WireGuard peers bound only to loopback. It also rejects mismatched families and waits for a pending IPv6 DNS dial during shutdown. Its inner IP addresses and listeners exist inside the userspace stacks, not on the host network. It accepts no configuration file, saved session, real account or remote peer, and does not run on normal app startup. A version 2 JSON report is printed only after the actual exchanges and shutdown checks succeed.

The packaged-binary acceptance script requires the exact version 2 report, including both IP families, A/AAAA DNS, the family gate and shutdown, as well as successful exit, clean stderr and an unchanged binary hash. An IPv4-only report cannot pass. Failed or timed-out checks replace any old passing evidence with a fixed failure code; raw helper output is not copied into reports.
...
Real Proton sign-in, two-factor/browser challenges, friend-run Mac acceptance, macOS 13 device acceptance, upgrade behavior and trusted signing remain unverified. The current native runners are macOS 14 on Apple Silicon and macOS 15 on Intel; a checked deployment target is not hardware acceptance. Before Connect can be enabled, networking still needs a supported macOS provider/deployment path, verified source-app identity and isolation, complete system DNS/IPv6 policy, real-server UDP behavior, network-change recovery and end-to-end disconnect/exit ownership. The loopback diagnostic does not establish any of those OS-level properties.
